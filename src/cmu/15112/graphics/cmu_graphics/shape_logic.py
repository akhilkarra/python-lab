...
from cmu_graphics import cmu_graphics, utils
...
class Shape(object):
    ...
    def set_rotateAngle(self, v):
        self.rotate(v - self.rotateAngle)
    ...
    def hits(self, *arguments):  # hits(x,y)
        ...
        if not utils.polygonContainsPoint(pts, x, y):
            return False
        if self.fill or isinstance(self, CMUImage):
            return True
        border = self.border
        if not border:
            return False
...
class Polygon(Shape):
    ...
    def get_left(self):
        return min(map(lambda point: point[0], self.pointList))
    ...
    def get_top(self):
        return min(map(lambda point: point[1], self.pointList))
...
class CMUSound(object):
    ...
    def sendProcessMessage(self, message):
        ...
        if (
            self.soundProcess.returncode is not None
            and self.soundProcess.returncode != 0
        ):
    ...
    def play(self, doLoop, doRestart):
        self.sendProcessMessage(
            {"command": "play", "kwargs": {"doLoop": doLoop, "doRestart": doRestart}}
        )

    def pause(self):
        self.sendProcessMessage({"command": "pause", "kwargs": {}})
...
class Oval(PolygonWithTransform):
    def __init__(self, attrs):
        ...
        attrs["bezierPoints"] = Oval.getBezierPoints(
            attrs["startAngle"], attrs["sweepAngle"]
        )
        super().__init__(attrs)
    ...
    def getBezierPoints(startAngle, sweepAngle):
        ...
        while remaining > 0:
            bp.extend(
                Oval.getBezierFragment(utils.toRadians(min(90, remaining)), offset)
            )
            offset += math.pi / 2
            remaining -= 90
        return bp
...
class Arc(Oval):
    ...
    def regeneratePoints(self):
        self.pointList = utils.getArcPoints(
            0, 0, 2, 2, self.startAngle, self.sweepAngle, (self.width + self.height) / 2
        )
...
class Inspector(object):
    ...
    def draw(self, ctx):
        ...
        pointLabelCenterX = min(
            400 - margin - w / 2, max(margin + w / 2, self.bestX - 10)
        )
        pointLabelCenterY = min(
            400 - margin - h / 2, max(margin + h / 2, self.bestY - 10)
        )
